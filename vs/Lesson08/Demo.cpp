...
void Demo::BuildTexturedCube()
{
	...
	int width, height;
	unsigned char* image = SOIL_load_image("woodgrain_diffuse.jpg", &width, &height, 0, SOIL_LOAD_RGBA);
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	image = SOIL_load_image("wood_hr.jpg", &width, &height, 0, SOIL_LOAD_RGBA);
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, image);
	SOIL_free_image_data(image);
	glBindTexture(GL_TEXTURE_2D, 0);
	...
	float vertices[] = {
		...
		-0.5,  0.5, 0.5, 0, 1, 0.0f,  0.0f,  1.0f, // 3
		 ...
		 0.5,  0.5, 0.4, 1, 0, 1.0f,  0.0f,  0.0f, // 5
		...
		0.5,   0.5, 0.4, 1, 1, 0.0f,  0.0f,  -1.0f, // 10
		-0.5,  0.5, 0.4, 0, 1, 0.0f,  0.0f,  -1.0f, // 11

		// left
		-0.5, -0.5, 0.4, 0, 0, -1.0f,  0.0f,  0.0f, // 12
		-0.5, -0.5,  0.5, 1, 0, -1.0f,  0.0f,  0.0f, // 13
		-0.5,  0.5,  0.5, 1, 1, -1.0f,  0.0f,  0.0f, // 14
		-0.5,  0.5, 0.4, 0, 1, -1.0f,  0.0f,  0.0f, // 15

		// upper
		0.5, 0.5,  0.5, 0, 0,   0.0f,  1.0f,  0.0f, // 16
		-0.5, 0.5, 0.5, 1, 0,   0.0f,  1.0f,  0.0f, // 17
		-0.5, 0.5, 0.4, 1, 1,  0.0f,  1.0f,  0.0f, // 18
		0.5, 0.5, 0.4, 0, 1,   0.0f,  1.0f,  0.0f, // 19
		...
		// right
		0.5,  0.5, -0.6, 0, 0, 1.0f,  0.0f,  0.0f, // 28
		0.5,  0.5, -0.5, 1, 0, 1.0f,  0.0f,  0.0f, // 29
		0.5, -0.5, -0.5, 1, 1, 1.0f,  0.0f,  0.0f, // 30
		0.5, -0.5, -0.6, 0, 1, 1.0f,  0.0f,  0.0f, // 31

		// back
		0.4, -0.5, -0.6, 0, 0, 0.0f,  0.0f,  -1.0f,// 32 
		0.5,  -0.5, -0.6, 1, 0, 0.0f,  0.0f,  -1.0f,// 33
		0.5,   0.5, -0.6, 1, 1, 0.0f,  0.0f,  -1.0f,// 34
		0.4,  0.5, -0.6, 0, 1, 0.0f,  0.0f,  -1.0f,// 35

		// left
		0.4, -0.5, -0.5, 0, 0, -1.0f,  0.0f,  0.0f,// 36
		0.4, -0.5, -0.6, 1, 0, -1.0f,  0.0f,  0.0f,// 37
		0.4,  0.5, -0.6, 1, 1, -1.0f,  0.0f,  0.0f,// 38
		0.4,  0.5, -0.5, 0, 1, -1.0f,  0.0f,  0.0f,// 39

		// upper
		0.5, 0.5,  0.5, 0, 0, 0.0f,  1.0f,  0.0f,  // 40
		-0.5, 0.5,  0.5, 1, 0, 0.0f,  1.0f,  0.0f, // 41
		-0.5, 0.5, 0.4, 1, 1, 0.0f,  1.0f,  0.0f, // 42
		0.5, 0.5, 0.4, 0, 1, 0.0f,  1.0f,  0.0f,  // 43

		// bottom
		-0.5, -0.5, 0.4, 0, 0, 0.0f,  -1.0f,  0.0f,// 44
		0.5, -0.5, 0.4, 1, 0, 0.0f,  -1.0f,  0.0f, // 45
		0.5, -0.5,  0.5, 1, 1, 0.0f,  -1.0f,  0.0f, // 46
		-0.5, -0.5,  0.5, 0, 1, 0.0f,  -1.0f,  0.0f,// 47

		// upnext (tiang kiri belakang)

		// front
		-0.5, -0.5, -0.5, 0, 0, 0.0f,  0.0f,  1.0f, // 48
		-0.4, -0.5, -0.5, 1, 0, 0.0f,  0.0f,  1.0f,  // 49
		-0.4,  0.5, -0.5, 1, 1, 0.0f,  0.0f,  1.0f,  // 50
		-0.5,  0.5, -0.5, 0, 1, 0.0f,  0.0f,  1.0f, // 51

		// right
		-0.4,  0.5, -0.6, 0, 0, 1.0f,  0.0f,  0.0f, // 52
		-0.4,  0.5, -0.5, 1, 0, 1.0f,  0.0f,  0.0f, // 53
		-0.4, -0.5, -0.5, 1, 1, 1.0f,  0.0f,  0.0f, // 54
		-0.4, -0.5, -0.6, 0, 1, 1.0f,  0.0f,  0.0f, // 55

		// back
		-0.5, -0.5, -0.6, 0, 0, 0.0f,  0.0f,  -1.0f,// 56
		-0.4,  -0.5, -0.6, 1, 0, 0.0f,  0.0f,  -1.0f,// 57
		-0.4,   0.5, -0.6, 1, 1, 0.0f,  0.0f,  -1.0f,// 58
		-0.5,  0.5, -0.6, 0, 1, 0.0f,  0.0f,  -1.0f,// 59

		// left
		-0.5, -0.5, -0.5, 0, 0, -1.0f,  0.0f,  0.0f,// 60
		-0.5, -0.5, -0.6, 1, 0, -1.0f,  0.0f,  0.0f,// 61
		-0.5,  0.5, -0.6, 1, 1, -1.0f,  0.0f,  0.0f,// 62
		-0.5,  0.5, -0.5, 0, 1, -1.0f, 0.0f, 0.0f,// 63

		// upper
		0.5, 0.5,  0.5, 0, 0, 0.0f, 1.0f, 0.0f,  // 64
		-0.5, 0.5,  0.5, 1, 0, 0.0f, 1.0f, 0.0f, // 65
		-0.5, 0.5, 0.4, 1, 1, 0.0f, 1.0f, 0.0f, // 66
		0.5, 0.5, 0.4, 0, 1, 0.0f, 1.0f, 0.0f, // 67

		// bottom
		-0.5, -0.5, 0.4, 0, 0, 0.0f, -1.0f, 0.0f,// 68
		0.5, -0.5, 0.4, 1, 0, 0.0f, -1.0f, 0.0f, // 69
		0.5, -0.5,  0.5, 1, 1, 0.0f, -1.0f, 0.0f, // 70
		-0.5, -0.5,  0.5, 0, 1, 0.0f, -1.0f, 0.0f,// 71

		// upnext (tempat duduk)

		// front
		-0.5, 0.5, 0.5, 0, 0, 0.0f, 0.0f, 1.0f, // 72
		0.5, 0.5, 0.5, 1, 0, 0.0f, 0.0f, 1.0f,  // 73
		0.5, 0.6, 0.5, 1, 1, 0.0f, 0.0f, 1.0f,  // 74
		-0.5, 0.6, 0.5, 0, 1, 0.0f, 0.0f, 1.0f, // 75

		// right
		0.5, 0.6, 0.5, 0, 0, 1.0f, 0.0f, 0.0f, // 76
		0.5, 0.6, -0.5, 1, 0, 1.0f, 0.0f, 0.0f, // 77
		0.5, 0.5, -0.5, 1, 1, 1.0f, 0.0f, 0.0f, // 78
		0.5, 0.5, 0.5, 0, 1, 1.0f, 0.0f, 0.0f, // 79

		// back
		-0.5, 0.5, -0.5, 0, 0, 0.0f, 0.0f, -1.0f,// 80 
		0.5, 0.5, -0.5, 1, 0, 0.0f, 0.0f, -1.0f,// 81
		0.5, 0.6, -0.5, 1, 1, 0.0f, 0.0f, -1.0f,// 82
		-0.5, 0.6, -0.5, 0, 1, 0.0f, 0.0f, -1.0f,// 83

		// left
		-0.5, 0.5, -0.5, 0, 0, -1.0f, 0.0f, 0.0f,// 84
		-0.5, 0.5, 0.5, 1, 0, -1.0f, 0.0f, 0.0f,// 85
		-0.5, 0.6, 0.5, 1, 1, -1.0f, 0.0f, 0.0f,// 86
		-0.5, 0.6, -0.5, 0, 1, -1.0f, 0.0f, 0.0f,// 87

		// upper
		0.5, 0.6, 0.5, 0, 0, 0.0f, 1.0f, 0.0f,  // 88
		-0.5, 0.6, 0.5, 1, 0, 0.0f, 1.0f, 0.0f, // 89
		-0.5, 0.6, -0.5, 1, 1, 0.0f, 1.0f, 0.0f, // 90
		0.5, 0.6, -0.5, 0, 1, 0.0f, 1.0f, 0.0f,  // 91

		// bottom
		-0.5, 0.5, -0.5, 0, 0, 0.0f, -1.0f, 0.0f,// 92
		0.5, 0.5, -0.5, 1, 0, 0.0f, -1.0f, 0.0f, // 93
		0.5, 0.5, 0.5, 1, 1, 0.0f, -1.0f, 0.0f, // 94
		-0.5, 0.5, 0.5, 0, 1, 0.0f, -1.0f, 0.0f,// 95

		// upnext (sandaran)

		// front
		-0.5, 0.5, -0.5, 0, 0, 0.0f, 0.0f, 1.0f, // 96
		0.5, 0.5, -0.5, 1, 0, 0.0f, 0.0f, 1.0f,  // 97
		0.5, 1.6, -0.5, 1, 1, 0.0f, 0.0f, 1.0f,  // 98
		-0.5, 1.6, -0.5, 0, 1, 0.0f, 0.0f, 1.0f, // 99

		// right
		0.5, 1.6, -0.6, 0, 0, 1.0f, 0.0f, 0.0f, // 100
		0.5, 1.6, -0.5, 1, 0, 1.0f, 0.0f, 0.0f, // 101
		0.5, 0.5, -0.5, 1, 1, 1.0f, 0.0f, 0.0f,// 102
		0.5, 0.5, -0.6, 0, 1, 1.0f, 0.0f, 0.0f, // 103

		// back
		-0.5, 0.5, -0.6, 0, 0, 0.0f, 0.0f, -1.0f,// 104 
		0.5, 0.5, -0.6, 1, 0, 0.0f, 0.0f, -1.0f,// 105
		0.5, 1.6, -0.6, 1, 1, 0.0f, 0.0f, -1.0f,// 106
		-0.5, 1.6, -0.6, 0, 1, 0.0f, 0.0f, -1.0f,// 107

		// left
		-0.5, 0.5, -0.5, 0, 0, -1.0f, 0.0f, 0.0f,// 108
		-0.5, 0.5, -0.6, 1, 0, -1.0f, 0.0f, 0.0f,// 109
		-0.5, 1.6, -0.6, 1, 1, -1.0f, 0.0f, 0.0f,// 110
		-0.5, 1.6, -0.5, 0, 1, -1.0f, 0.0f, 0.0f,// 111

		// upper
		0.5, 0.5, 0.5, 0, 0, 0.0f, 1.0f, 0.0f,  // 112
		-0.5, 0.5, 0.5, 1, 0, 0.0f, 1.0f, 0.0f, // 113
		-0.5, 0.5, 0.4, 1, 1, 0.0f, 1.0f, 0.0f, // 114
		0.5, 0.5, 0.4, 0, 1, 0.0f, 1.0f, 0.0f,  // 115

		// bottom
		-0.5, -0.5, 0.4, 0, 0, 0.0f, -1.0f, 0.0f,// 116
		0.5, -0.5, 0.4, 1, 0, 0.0f, -1.0f, 0.0f, // 117
		0.5, -0.5, 0.5, 1, 1, 0.0f, -1.0f, 0.0f, // 118
		-0.5, -0.5, 0.5, 0, 1, 0.0f, -1.0f, 0.0f,// 119
	};
